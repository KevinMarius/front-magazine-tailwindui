...
import bgColorState from '../utils/helpers';
...
function PostList(props) {
    ...
    const confirmDeleteHandler = async (e, data) => {
        ...
        try {
            const response = await data.forEach(element => {
                sendRequest(`http://localhost:3500/api/post/delete/${element.original._id}`,
                    'delete', null, {
                    "Authorization": "Bearer " + token
                });
                props.postDeleteHandle(element.original._id);
            });
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            Header: "Category",
            Footer: "Category",
            accessor: "categoryId.title",
            Cell: ({ row: { original } }) => (
                <span
                    className='bg-red-200 p-1 rounded-lg text-gray-700'
                >
                    {original.categoryId.title}
                </span>
            ),
        },
        {
            Header: "Status",
            Footer: "Status",
            accessor: "stateId.title",
            Cell: ({ row: { original } }) => (
                bgColorState(original.stateId.title)
            ),
        },
        {
            Header: 'Action',
            ...
            accessor: (originalRow, rowIndex) => (
                <div>
                    <Link className='text-xs underline text-blue-700' to={`/dashboard/post/${originalRow._id}`}>Edit</Link>
                </div>
            ),
            id: 'action',
        }
