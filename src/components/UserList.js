    ...
    const confirmDeleteHandler = async (e, data) => {
        ...
        try {
            const response = await data.forEach(element => {
                sendRequest(`http://localhost:3500/api/user/delete/${element.original._id}`,
                'delete', null, {
                "Authorization": "Bearer " + token
            });
            props.userDeleteHandle(element.original._id);
            });
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            Cell: ({ row: { original } }) => (
                ...
                >
                    <img className='h-10 w-10 rounded-full' src={`http://localhost:3500/${original.picture}`} />
                    {original.name}
                </div>
            ),
        },
        {
            ...
            accessor: "role.title",
            Cell: ({ row: { original } }) => (
                <span className='text-orange-600 text-xs font-extrabold'>
                    {original.role.title}
                </span>
            ),
        },
        ...
        },
        {
            ...
            accessor: (originalRow, rowIndex) => (
                <div>
                    <Link className='text-xs underline text-blue-700' to={`/dashboard/user/${originalRow._id}`}>Edit</Link>
                </div>
            ),
            id: 'action',
        }
                    ...
                    <div className="flex flex-col px-4 py-1 space-y-3 md:flex-row md:items-center md:justify-between md:space-y-0 md:space-x-4">
                        <div className="flex items-center flex-1 space-x-4">
                            <h5 className='flex gap-2'>
                                <span className="text-gray-500 text-md font-semibold">All users:</span>
                        ...
                        <div className="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                            <Link to='/dashboard/adduser' className="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-orange-700 hover:bg-orange-800 focus:ring-4 focus:ring-orange-300 dark:bg-orange-600 dark:hover:bg-orange-700 focus:outline-none dark:focus:ring-orange-800">
