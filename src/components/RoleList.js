...
function RoleList(props) {
	...
	const confirmDeleteHandler = async (e, data) => {
		...
		try {
			const response = await data.forEach(element => {
				sendRequest(`http://localhost:3500/api/role/delete/${element.original._id}`,
					'delete', null, {
					"Authorization": "Bearer " + token
				});
				props.roleDeleteHandle(element.original._id);
			});
	...
	const columns = useMemo(() => [
		...
		},
		{
			...
			accessor: (originalRow, rowIndex) => (
				<div>
					<Link className='text-xs underline text-blue-700' to={`/dashboard/role/${originalRow._id}`}>Edit</Link>
				</div>
			),
			id: 'action',
		}
					...
					<div className="flex flex-col px-4 py-1 space-y-3 md:flex-row md:items-center md:justify-between md:space-y-0 md:space-x-4">
						<div className="flex items-center flex-1 space-x-4">
							<h5 className='flex gap-2'>
								<span className="text-gray-500 text-md font-semibold">All Roles:</span>
						...
						<div className="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
							<Link to='/dashboard/addrole' className="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-orange-700 hover:bg-orange-800 focus:ring-4 focus:ring-orange-300 dark:bg-orange-600 dark:hover:bg-orange-700 focus:outline-none dark:focus:ring-orange-800">
